export class APIUtils
{
    apiContext: any;
    loginPayload: string;
constructor(apiContext:any,loginPayload:string) //so that it gets used throughout this class, Login would be mandatory steps for all flows
{
...
async getToken()
{
    const apiResponse:any= await this.apiContext.post("https://rahulshettyacademy.com/api/ecom/auth/login", 
        {
...
/* Syntax	Meaning
token: string	property required
token?: string	property optional */
async createOrder(orderPayload:string){ //specifically passed here and not in constructor as it is method specific
let response: {token?: string, orderID?: string}={}; //response is an object whose properties are strings//token: String means token contains string class function
response.token = await this.getToken(); //response objects has been assigned token property in this step
   //Either above or this let response:{token?: string, orderID?: string}={}; //JS object //? optional property as it may or may not be present in the response of the API call. This is especially useful when we are not sure about the properties that will be present in the response of the API call. We can also use other types like string, number, etc. depending on our needs  response.token= await this.getToken(); //response objects has been assigned token property in this step
    //url we can get from network tab before placing order 
    const orderApiResponse:any= await this.apiContext.post(" https://rahulshettyacademy.com/api/ecom/order/create-order",
        {
        data:orderPayload,
        headers: {
